...
const Exams = () => {
    ...
    useEffect(() => {
        fetch(`https://desolate-wildwood-64014.herokuapp.com/question?category=${name}`)
            .then(res => res.json())
            ...
            {(minutes === 0 && seconds === 0) || showScore
                ? (
                    <div className='score-section p-5 rounded'>
                        <h2>Exam is finished</h2>
                        <h4 className='mt-4'>You scored {score} out of {questions.length}</h4>
                        ...
                        <br />
                        <button className='btn btn-warning mt-3 me-3' onClick={() => navigate(`/all-questions/${name}`)}>View All Answers</button>
